...
async function load() {
  loading.value = true
  try {
    const page = await getMyOvertimeRequests({ all: true, page: 1, pageSize: 100, status: 'pending' })
...
async function review(r: OvertimeRequest, action: 'approve' | 'reject') {
  const comment = action === 'reject' ? (window.prompt('請填寫駁回原因', '') ?? '') : ''
  if (action === 'reject' && !comment) {
    showToast('駁回必須填寫原因')
    return
  }
  if (!window.confirm(`確定${action === 'approve' ? '核准' : '駁回'}「${r.employeeName}」的加班申請？`)) return
  try {
    await reviewOvertimeRequest(r.id, action, comment)
...
<template>
  <section class="card">
    <h2>加班審核</h2>
    <p class="muted">待審核的加班申請（HR/主管可查看與審核職權範圍內申請）</p>
    <div class="toolbar">
      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr v-if="!loading && items.length === 0">
            <td colspan="9" class="muted">目前沒有待審核的加班申請</td>
          </tr>
        </tbody>
      </table>
    </div>
